...
app = Flask(__name__)
MONGODBHOST="mongodb://xxx:xxx@127.0.0.1:27017/IdleFish"
...
@app.before_request
def before_action():
    if request.path==baseurl+"/login" or request.path==baseurl+"/squery" :
        return
...
def login():
    ...
    if loginname=="dongkirk" and password=="githubisNo.1":
        token = jwt.encode({'loginname':loginname,"ctime":int(time.time()),'expires':60*60*24*7}, jwtsecret, algorithm='HS256')
        # print(token)
        return jsonify({
            "status": 0,
            "msg":"登陆成功",
